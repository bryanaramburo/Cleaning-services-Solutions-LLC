...
// 📁 IMAGE CONFIGURATION
// ============================================================
// HOW TO ADD YOUR PHOTOS IN GITHUB:
//
// 1. In your GitHub repo, create this folder: public/images/
// 2. Upload your files into that folder
// 3. Update the file names below to match what you uploaded
//
// LOGO:    public/images/logo.png      (transparent background PNG)
// HERO:    public/images/hero.jpg      (any clean home photo)
// ABOUT:   public/images/maria.jpg     (photo of Maria)
// GALLERY: public/images/gallery-1.jpg through gallery-12.jpg
//
// TIP: Export the logo as PNG with transparent background so it
// looks clean on both the white header and dark navy footer.
// ============================================================

const IMAGES = {
  // Company logo — appears in header and footer
  logo: "/images/logo.png",

  // Hero section — large photo on the right side of the headline
  hero: "/images/hero.jpg",

  // About Us section — photo of Maria
  about: "/images/maria.jpg",

  // Gallery photos — add or remove entries as needed
  // Format: { src: "/images/your-file.jpg", alt: "Description" }
  gallery: [
    { src: "/images/gallery-1.jpg",  alt: "Clean Kitchen" },
    { src: "/images/gallery-2.jpg",  alt: "Sparkling Bathroom" },
    { src: "/images/gallery-3.jpg",  alt: "Tidy Bedroom" },
    { src: "/images/gallery-4.jpg",  alt: "Clean Living Room" },
    { src: "/images/gallery-5.jpg",  alt: "Detailed Cleaning" },
    { src: "/images/gallery-6.jpg",  alt: "Spotless Floors" },
    { src: "/images/gallery-7.jpg",  alt: "Polished Surfaces" },
    { src: "/images/gallery-8.jpg",  alt: "Fresh Linens" },
    { src: "/images/gallery-9.jpg",  alt: "Window Cleaning" },
    { src: "/images/gallery-10.jpg", alt: "Modern Bathroom" },
    { src: "/images/gallery-11.jpg", alt: "Kitchen Detail" },
    { src: "/images/gallery-12.jpg", alt: "Organized Space" },
    // To add more photos: copy the line above, paste below, update filename + alt text
  ],
};
...
// Shows a styled placeholder when an image file hasn't been uploaded yet
const ImgPlaceholder = ({ label, className = "" }: { label: string; className?: string }) => (
  <div className={`flex flex-col items-center justify-center bg-brand-mint/30 border-2 border-dashed border-brand-blue/30 rounded-2xl text-brand-blue/50 text-center p-6 ${className}`}>
    ...
    <span className="text-xs opacity-40 mt-1">Add to public/images/</span>
  </div>
);

// Smart image — automatically shows a placeholder if the file hasn't been uploaded yet
const SiteImage = ({ src, alt, className = "" }: { src: string; alt: string; className?: string }) => {
  const [errored, setErrored] = React.useState(false);
  if (errored) return <ImgPlaceholder label={alt} className={className} />;
  return (
    <img
      src={src}
      alt={alt}
...
const Logo = ({ size = "md", dark = false }: { size?: "sm" | "md" | "lg"; dark?: boolean }) => {
  ...
  if (errored) {
    return (
      <div className="flex items-center gap-2">
        ...
        <span className={`text-xl md:text-2xl font-display font-bold leading-tight ${dark ? 'text-white' : 'text-brand-blue'}`}>
          Cleaning Service<br />
          <span className={`text-sm md:text-base font-medium tracking-wide opacity-80 ${dark ? 'text-white' : 'text-brand-navy'}`}>
            Solutions LLC
          </span>
        </span>
      </div>
  ...
  return (
    <img
      src={IMAGES.logo}
      alt="Cleaning Service Solutions LLC"
...
const Sparkle = ({ className = "" }: { className?: string }) => (
  <motion.div
    ...
    animate={{ scale: [0, 1.2, 1], opacity: [0, 1, 0.6], rotate: [0, 45, 90] }}
    ...
    <div className="min-h-screen font-sans selection:bg-brand-green selection:text-brand-navy">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-3' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          {/* HEADER LOGO — upload public/images/logo.png to swap the icon fallback */}
          <Logo size="md" />

          {/* Desktop Nav */}
          <div className="hidden lg:flex items-center gap-8">
            {['Services', 'Gallery', 'Why Us', 'About', 'Contact'].map((item) => (
              <a key={item} href={`#${item.toLowerCase().replace(' ', '')}`} className="text-brand-navy font-medium hover:text-brand-blue transition-colors min-h-[44px] flex items-center">
                {item}
              </a>
            ))}
            <div className="flex gap-2">
              <a href="tel:7703770635" className="bg-brand-blue text-white px-5 py-2 rounded-full font-bold flex items-center gap-2 hover:bg-brand-green hover:text-brand-navy transition-all shadow-lg shadow-brand-blue/20 min-h-[44px]">
                <Phone size={16} /> Call Us
              </a>
              <a href="sms:7703770635" className="bg-brand-blue text-white px-5 py-2 rounded-full font-bold flex items-center gap-2 hover:bg-brand-green hover:text-brand-navy transition-all shadow-lg shadow-brand-blue/20 min-h-[44px]">
          ...
          {/* Mobile Toggle */}
          <button className="lg:hidden text-brand-blue p-2 min-h-[44px] min-w-[44px] flex items-center justify-center" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Toggle menu">
          ...
          {isMenuOpen && (
            ...
            >
              <div className="px-6 py-8 flex flex-col gap-6">
                {['Services', 'Gallery', 'Why Us', 'About', 'Contact'].map((item) => (
                  <a key={item} href={`#${item.toLowerCase().replace(' ', '')}`} onClick={() => setIsMenuOpen(false)} className="text-xl font-display font-semibold text-brand-navy min-h-[44px] flex items-center">
                    {item}
                  </a>
                ))}
                <div className="flex flex-col gap-4 mt-4">
                  <div className="flex gap-2">
                    <a href="tel:7703770635" className="flex-1 bg-brand-blue text-white px-6 py-4 rounded-xl font-bold flex items-center justify-center gap-2 min-h-[44px]"><Phone size={20} /> Call Us</a>
                    <a href="sms:7703770635" className="flex-1 bg-brand-blue text-white px-6 py-4 rounded-xl font-bold flex items-center justify-center gap-2 min-h-[44px]"><MessageSquare size={20} /> Text Us</a>
                  </div>
                  <a href="mailto:24cleaningcolutions@gmail.com" className="bg-brand-blue text-white px-6 py-4 rounded-xl font-bold flex items-center justify-center gap-2 min-h-[44px]"><Mail size={20} /> Email Us</a>
                </div>
              </div>
            </motion.div>
          )}
      ...
      <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden bg-brand-mint/30">
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <a href="#contact" className="bg-white text-brand-blue border-2 border-brand-blue px-8 py-4 rounded-full font-bold text-lg flex items-center justify-center gap-2 hover:bg-brand-mint/20 transition-all">
                Get a Free Quote <ArrowRight size={22} />
              </a>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }} className="relative">
            {/* HERO IMAGE — upload public/images/hero.jpg to replace */}
            <div className="relative rounded-3xl overflow-hidden shadow-2xl border-8 border-white">
              <SiteImage src={IMAGES.hero} alt="Professional home cleaning" className="w-full h-[400px] md:h-[500px] object-cover" />
      ...
      <section id="gallery" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <SectionHeading title="Our Work in Action" subtitle="Gallery" />
          {/* GALLERY — upload public/images/gallery-1.jpg through gallery-12.jpg */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {IMAGES.gallery.map((image, idx) => (
              <motion.div
                ...
                className="group relative aspect-square rounded-2xl overflow-hidden shadow-md bg-brand-mint/20"
              >
                <SiteImage src={image.src} alt={image.alt} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section id="whyus" className="py-24 bg-brand-mint/20">
        <div className="max-w-7xl mx-auto px-6">
          <SectionHeading title="Why Homeowners Trust Us" subtitle="The CSS Difference" />
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-8">
            {trustPoints.map((point, idx) => (
              <motion.div key={point.title} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="text-center">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative">
              {/* ABOUT PHOTO — upload public/images/maria.jpg to replace */}
              <div className="aspect-square rounded-[40px] overflow-hidden shadow-2xl relative z-10">
                <SiteImage src={IMAGES.about} alt="Maria, Owner of Cleaning Service Solutions" className="w-full h-full object-cover" />
              </div>
            ...
            <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <div className="space-y-6 text-lg text-brand-navy/80 leading-relaxed">
                <p>Hi, I'm Maria! With over 5 years of professional cleaning experience right here in the Gainesville area, I founded Cleaning Service Solutions LLC to bring a higher standard of care to our community.</p>
                <p>As a family-owned and operated business, we treat every home as if it were our own. My passion for a spotless home is what drives us to be meticulous in every corner we clean.</p>
                <p>As a bilingual team, we proudly serve both English and Spanish-speaking households across North Georgia, ensuring clear communication and exceptional service for all our clients.</p>
                <p>We know that inviting someone into your home requires trust. That's why we're fully insured, bonded, and committed to providing a reliable, friendly service that makes your life easier and your home healthier.</p>
              </div>
      ...
      <section className="py-20 bg-brand-blue text-white overflow-hidden relative">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center relative z-10">
          <SectionHeading title="Where We Sparkle" subtitle="Service Areas" light />
          <div className="flex flex-wrap justify-center gap-4 md:gap-6 mb-12">
            {businessInfo.serviceAreas.map((area, idx) => (
              <motion.div key={area} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: idx * 0.05 }} className="bg-white/10 backdrop-blur-md border border-white/20 px-8 py-4 rounded-2xl font-display font-bold text-xl md:text-2xl hover:bg-brand-green hover:text-brand-navy transition-all cursor-default">
                {area}
              </motion.div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <p className="text-lg text-brand-navy/70 mb-10">Ready for a spotless home? Fill out the form below or reach out directly. We'll get back to you quickly with a personalized estimate.</p>
              <div className="space-y-8">
                <p className="text-brand-blue font-bold italic flex items-center gap-2">
                  <Languages size={20} /> Hablamos Español — feel free to call or text us in Spanish.
                </p>
                <div className="flex items-start gap-6">
                  <div className="bg-brand-mint/30 p-4 rounded-2xl text-brand-blue"><Phone size={28} /></div>
                  <div className="flex-1">
                    <h4 className="font-display font-bold text-xl text-brand-navy mb-3">Call or Text</h4>
                    <div className="flex flex-wrap gap-3">
                      <a href="tel:7703770635" className="bg-brand-blue text-white px-6 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-brand-green hover:text-brand-navy transition-all shadow-md min-h-[44px]"><Phone size={20} /> Call Us</a>
                      <a href="sms:7703770635" className="bg-brand-blue text-white px-6 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-brand-green hover:text-brand-navy transition-all shadow-md min-h-[44px]"><MessageSquare size={20} /> Text Us</a>
                    </div>
                  </div>
                </div>
                <div className="flex items-start gap-6">
                  <div className="bg-brand-mint/30 p-4 rounded-2xl text-brand-blue"><Mail size={28} /></div>
                  <div className="flex-1">
                    <h4 className="font-display font-bold text-xl text-brand-navy mb-3">Email Us</h4>
                    <a href="mailto:24cleaningcolutions@gmail.com" className="inline-flex bg-brand-blue text-white px-6 py-3 rounded-full font-bold items-center gap-2 hover:bg-brand-green hover:text-brand-navy transition-all shadow-md min-h-[44px]"><Mail size={20} /> Email Us</a>
                  </div>
                </div>
                <div className="flex items-start gap-6">
                  <div className="bg-brand-mint/30 p-4 rounded-2xl text-brand-blue"><Clock size={28} /></div>
                  <div>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="bg-brand-mint/10 p-8 md:p-12 rounded-[40px] border border-brand-mint/30 shadow-xl">
              <form onSubmit={handleFormSubmit} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-brand-navy uppercase tracking-wide">Full Name</label>
                    <input required name="name" type="text" placeholder="Jane Doe" className="w-full px-6 py-4 rounded-2xl border border-gray-200 focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/10 outline-none transition-all bg-white" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-brand-navy uppercase tracking-wide">Phone Number</label>
                    <input required name="phone" type="tel" placeholder="(770) 000-0000" className="w-full px-6 py-4 rounded-2xl border border-gray-200 focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/10 outline-none transition-all bg-white" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-brand-navy uppercase tracking-wide">Email Address</label>
                  <input required name="email" type="email" placeholder="jane@example.com" className="w-full px-6 py-4 rounded-2xl border border-gray-200 focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/10 outline-none transition-all bg-white" />
                </div>
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-brand-navy uppercase tracking-wide">Street Address</label>
                    <input required name="address" type="text" placeholder="123 Main St, Gainesville" className="w-full px-6 py-4 rounded-2xl border border-gray-200 focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/10 outline-none transition-all bg-white" />
                  </div>
                  <div className="space-y-2">
                ...
                <p className="text-center text-sm text-brand-navy/50 font-medium">Submitting will open your messaging app to send us these details.</p>
              </form>
            </motion.div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            <div className="col-span-1 lg:col-span-2">
              {/* FOOTER LOGO — same logo file used automatically */}
              <div className="mb-6">
                <Logo size="lg" dark />
              </div>
              ...
              <p className="text-brand-green font-bold mb-8 flex items-center gap-2">
                <Languages size={18} /> Se Habla Español
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex gap-2">
                  <a href="tel:7703770635" className="flex-1 bg-brand-blue text-white px-4 py-2.5 rounded-full font-bold flex items-center justify-center gap-2 hover:bg-brand-green hover:text-brand-navy transition-all text-sm shadow-md min-h-[44px]"><Phone size={16} /> Call Us</a>
                  <a href="sms:7703770635" className="flex-1 bg-brand-blue text-white px-4 py-2.5 rounded-full font-bold flex items-center justify-center gap-2 hover:bg-brand-green hover:text-brand-navy transition-all text-sm shadow-md min-h-[44px]"><MessageSquare size={16} /> Text Us</a>
                </div>
                <a href="mailto:24cleaningcolutions@gmail.com" className="bg-brand-blue text-white px-4 py-2.5 rounded-full font-bold flex items-center justify-center gap-2 hover:bg-brand-green hover:text-brand-navy transition-all text-sm shadow-md min-h-[44px]"><Mail size={16} /> Email Us</a>
              </div>
            </div>
